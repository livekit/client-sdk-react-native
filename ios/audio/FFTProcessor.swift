...
public class FFTResult {
  ...
  func computeBands(minFrequency: Float, maxFrequency: Float, bandsCount: Int, sampleRate: Float)
    ...
    return realBuffer.withUnsafeMutableBufferPointer { realPtr in
      imaginaryBuffer.withUnsafeMutableBufferPointer { imagPtr in
        ...
        windowedBuffer.withUnsafeBufferPointer { bufferPtr in
          let complexPtr = UnsafeRawPointer(bufferPtr.baseAddress!).bindMemory(
            to: DSPComplex.self, capacity: Int(bufferHalfSize))
